...
RegisterNetEvent("zGangsBuilder:CreateGangs")
AddEventHandler("zGangsBuilder:CreateGangs", function(data)
...
AddEventHandler("zGangsBuilder:PlayerSpawned", function()
    local _source = source 
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    if xJob.name == "unemployed" or xJob.name == "unemployed2" then 
        ...
        TriggerClientEvent("zGangsBuilder:PlayerSpawned", _source, SharedGangs[xJob.name])
    end
end)

RegisterNetEvent("zGangsBuilder:PlayerRefresh")
AddEventHandler("zGangsBuilder:PlayerRefresh", function()
    local _source = source 
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xJob = xPlayer.job2
    
    if xJob.name == "unemployed" or xJob.name == "unemployed2" then 
        TriggerClientEvent("zGangsBuilder:PlayerRefresh", _source, false)
    else
        TriggerClientEvent("zGangsBuilder:PlayerRefresh", _source, true, SharedGangs[xJob.name])
    end
end)

RegisterNetEvent("zGangsBuilder:AddVehicle")
AddEventHandler("zGangsBuilder:AddVehicle", function(job, vProps, vName)
    local _source = source 
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xJob = xPlayer.job2
...
RegisterNetEvent("zGangsBuilder:SuppVehicle")
AddEventHandler("zGangsBuilder:SuppVehicle", function(job, vPlate, vName)
    local _source = source 
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xJob = xPlayer.job2
    ...
    if xJob.name == job then 
        if SharedGangs[job].vehicle[vPlate] then 
            TriggerClientEvent("zGangsBuilder:SpawnVehicle", _source, SharedGangs[job].vehicle[vPlate])
...
ESX.RegisterServerCallback('zGangsBuilder:GetVehiclesGangs', function(source, cb, job)
    local _source = source 
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xJob = xPlayer.job2
...
RegisterNetEvent("zGangsBuilder:AddItem")
AddEventHandler("zGangsBuilder:AddItem", function(job, itemName, count)
    local _source = source 
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xJob = xPlayer.job2
        ...
        if SharedGangs[job].data["item"] then 
            ...
            if itemInfo.count >= count then 
                ...
                if (Config.CoffrePoidsMax - CurrentWeight) >= count then
                    ...
                    TriggerClientEvent("zGangsBuilder:UpdateCoffre", _source, SharedGangs[job].data)
                else 
...
RegisterNetEvent("zGangsBuilder:SuppItem")
AddEventHandler("zGangsBuilder:SuppItem", function(job, itemName, count)
    local _source = source 
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xJob = xPlayer.job2
...
RegisterNetEvent("zGangsBuilder:AddMoney")
AddEventHandler("zGangsBuilder:AddMoney", function(job, moneyType, amount) 
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xJob = xPlayer.job2
...
RegisterNetEvent("zGangsBuilder:SuppMoney")
AddEventHandler("zGangsBuilder:SuppMoney", function(job, moneyType, amount) 
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xJob = xPlayer.job2
...
RegisterNetEvent("zGangsBuilder:AddWeapons")
AddEventHandler("zGangsBuilder:AddWeapons", function(job, weaponName, ammo)
    local _source = source 
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xJob = xPlayer.job2
...
RegisterNetEvent("zGangsBuilder:SuppWeapons")
AddEventHandler("zGangsBuilder:SuppWeapons", function(job, keyWeapon, weaponName, ammo)
    local _source = source 
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xJob = xPlayer.job2
...
ESX.RegisterServerCallback('zGangsBuilder:GetItemGangs', function(source, cb, job)
    local _source = source 
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xJob = xPlayer.job2
...
RegisterServerEvent('zGangsBuilder:playerOpenedCoffre')
AddEventHandler('zGangsBuilder:playerOpenedCoffre', function(gangsName)
    local _source = source 
...
RegisterServerEvent('zGangsBuilder:playerClosedCoffre')
AddEventHandler('zGangsBuilder:playerClosedCoffre', function(gangsName)
    local _source = source 
...
ESX.RegisterServerCallback('zGangsBuilder:CheckIfPlayerInCoffre', function(source, cb, gangsName)
    local _source = source
...
RegisterServerEvent('zGangsBuilder:playerOpenedGarage')
AddEventHandler('zGangsBuilder:playerOpenedGarage', function(gangsName)
    local _source = source 
...
RegisterServerEvent('zGangsBuilder:playerClosedGarage')
AddEventHandler('zGangsBuilder:playerClosedGarage', function(gangsName)
    local _source = source 
...
ESX.RegisterServerCallback('zGangsBuilder:CheckIfPlayerInGarage', function(source, cb, gangsName)
    local _source = source
...
RegisterCommand(Config.CommandName, function(source)
    ...
    if hasPerm then 
        TriggerClientEvent("zGangsBuilder:OpenMenuCreator", _source)
    else 
        return
    end
end)
